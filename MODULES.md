...
| 7 | `stages/masking.py` | Locked · reworked | v0.7-masking | **Single-phase** habitat mask. CoRE Stack LULC_v4 (IndiaSAT legend, classes 6 = Trees, 12 = Shrubs/Scrubs), majority vote over usable years with a most-recent-year tie-break; ESA WorldCover v200 (10/20/30) where IndiaSAT has no data. Water, cropland and built-up are excluded by *not being in the habitat set* — there is no separate subtraction. JRC GSW is loaded only for `water_mask`, which feeds distance-to-water. No VIIRS and no Open Buildings anywhere in `src/`; the `use_viirs` / `use_open_buildings` toggles were removed in the single-phase rework and `tests/test_config.py:213` asserts they are rejected |
...
| 14 | `stages/segmentation.py` | Locked · reworked v1.2.0 | v0.14-segmentation | SNIC superpixels. v1.2.0: input stack is **config-driven** (`segmentation.input_bands`), default five bands (B4, B8, canopy_height, ndvi_amplitude_annual, VV-VH) — `composite_nirv` dropped as an algebraic function of B4/B8, phenology added. `canopy_height_std` (canopy roughness) was added in v1.2.0 and removed from the default after the Odisha field validation (R² = 0.000 against crown cover; see `docs/datasets.md`); `features_structure` still produces it and it remains declarable. Boundaries are **no longer held identical across arms**: each arm segments on its own feature space. New `normalize_distance_scale` makes `compactness` comparable across stacks of different width |
| 14b | `stages/merge.py` | Locked | v1.2.0 | **NEW.** SNIC superpixels → forest stands, Xiong et al. 2024 §2.6: two passes, hard conjunctive gate on canopy_height / ndvi_amplitude_annual in physical units (default criteria; `canopy_height_std` was a third until the Odisha field validation, and with two criteria and `min_defined_criteria: 2` both must be defined for a pass-1 merge), hard area bounds, shared-edge fallback. Runs client-side over the adjacency graph, returns `snic_clusters.remap(...)`. Merge rule held identical across arms so delineation is the only difference. Not cached (thresholds are the iteration variable) |
| 15 | `stages/clustering.py` | Locked · reworked v1.2.0 | v0.15-clustering | wekaKMeans on the per-**stand** feature stack (per-superpixel when `merge.enabled: false`). Cyclic decomposition, log-transform skewed bands (DEC-004), median/IQR scaling (DEC-003), preprocessing params cached as asset property. v1.2.0: fits on **every unit, one row each** — `n_training_samples` retired, since a pixel sample area-weighted every statistic computed from it. v1.2.x: `reduceConnectedComponents` gets the measured widest unit extent plus margin (capped at `max_component_pixels`) instead of the cap, because EE pads tiles by `maxSize` and wide band stacks ran out of memory; units are still aggregated whole; manifest adds `rcc_neighbourhood_px`, `widest_unit_extent_px` |
...
| 17 | `stages/export.py` | Locked · extended v1.2.0 | v0.17-export · extended v1.2.0 | v0.17: Drive GeoTIFF + run manifest (config, asset paths, clustering metadata, distribution). Asset inventory auto-discovered from the stage registry. v1.1.0: added two vector layers (`stands_snic`, `stands_dissolved`) in SHP + GeoJSON, configurable `drive_folder`, manifest moved from singular `drive_export` to `drive_exports` dict + new `vector_layers` section. **v1.2.0**: added `stands_merged` — one polygon per merged stand, the primary deliverable, written whenever `merge.enabled` (all four shipped configs). Unlike `stands_snic` it KEEPS stands clustering could not type, with a null `cluster_id`, so the coverage gap is visible in the layer rather than only in the log. Manifest records `n_stands` beside `n_features` because `reduceToVectors` emits one polygon per connected component, not per label |
