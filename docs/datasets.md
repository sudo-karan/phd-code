...
- **Cloud masking:** SCL classes `[3, 8, 9, 10]` dropped (cloud shadow,
  cloud medium prob, cloud high prob, thin cirrus). Per-image `CLOUDY_PIXEL_PERCENTAGE` filter caps scene-level cloud at 20%.
- **Two separate windows on the same collection:**
  - **phenology** (long, 8y): `dates.phenology` → harmonic regression in
    `features_optical`. Long window lets year-to-year anomalies average out
    when fitting the smooth seasonal cycle.
  - **optical_composite** (1y): `dates.optical_composite` → single static
    median composite that SNIC sees in `segmentation`. Different problem
    (need clean snapshot, not a time series).
...
- **Window:** 2017-01-01 → 2021-12-01. Hard cap at Dec 2021 because S1B
...
variance much more than any 3×3 / 5×5 spatial filter could
(~√N reduction), and spatial filters blur edges that SNIC needs.
...
10·log10(VV_linear / VH_linear)` — the difference IS the (linear-scale)
ratio. See DEC-016.
...
- **Used with neighborhood stats in features_structure**: std-dev and max
  in a 3×3 window capture local heterogeneity (mature even-aged stand =
  low std; regenerating patch or forest edge = high std).
...
- **Aspect is cyclic** — emitted as raw degrees in `features_static`;
...
- **Used in masking** for two things:
  - Vegetation keep-list (default classes `[10, 20, 30]` = trees, shrubs,
    grass) → contributes to `habitat_mask`.
  - Class 80 (permanent water) → redundant water source layered with JRC.
...
- **In masking:** `occurrence ≥ jrc_water_occurrence_threshold` (default
  50%) → permanent water → excluded from `habitat_mask`.
...
caching layer fixes this — once `built_up_mask` is exported to an asset,
visualization reads the static raster instead of recomputing live.
...
- **Used in features_static** as `annual_rainfall`: sum over the 30-year
  standard climatology window (1991-01-01 → 2020-12-31), divide by 30,
  unit = mm/year.
...
2. Check that band names match what the consuming stage expects. If they
   don't, you'll need to edit the stage (e.g., features_optical hardcodes
...
read the stage source — most stages are <300 lines.
