...
1. **Task 1 — VERDICT: DISQUALIFIED.** Steps 7B and 10 have both run against Earth Engine,
   ...
   **Reading `cover_code` as metres is licensed, not assumed.** The asset carries **one band,
   ...
   categorical code would spike on arbitrary integers; this does not. Of those 554,745 pixels
   **63.5% read zero**, at a non-zero mean of **4.82 m** — which is a fact about the sampled
   pixels, not about forest cover. The sample is the union of 274 discs of 500 m radius around
   plot centres, and in rural Dhenkanal, Angul and Koraput that takes in farmland, fallow,
   settlement and roads. The control discs show the confound directly: 100% non-zero at 15 m,
   falling to 62.1% at 500 m as the disc widens into non-forest. The verdict rests on the
   plot-level grids above and does not depend on this share.
